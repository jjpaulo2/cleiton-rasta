...
from oci import config, Response
...
class OracleIntegration:
    ...
    @cached_property
    def _usage(self):
        return UsageapiClient(self.config)
    ...
    async def is_machine_running(self) -> bool:
        response = await asyncio.to_thread(self._compute.get_instance, self.machine_id)
        if isinstance(response, Response):
            return response.data.lifecycle_state == "RUNNING"
        return False
    
    async def start_machine(self):
        await asyncio.to_thread(self._compute.instance_action, self.machine_id, "START")

    async def stop_machine(self):
        await asyncio.to_thread(self._compute.instance_action, self.machine_id, "SOFTSTOP")

    async def current_cost(self) -> tuple[float, str, str]:
        response = await asyncio.to_thread(self._usage.request_summarized_usages, self.usage_payload)
        if isinstance(response, Response):
